...
public class ProfileActivity extends KActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		((Button) findViewById(R.id.button_save_profile))
							...
							public void requestControllerDidFail(
								...
								if (exception instanceof RequestControllerException) {
									...
									if (ctrlException
										...
										// this case is not quite a fatal error.
										// if the email address is already
										// taken, an email will be sent to it to
										// allow the user to link this device
										// with his account.
										// that's why we'll show a success
										// dialog in this case.
										...
										// username may be invalid, taken or too
										// short
										if (ctrlException
									...
									// generic Exception
									dialogErrorMsg = exception
											.getLocalizedMessage();
									showDialog(DIALOG_ERROR);
								}

								// update displayed values
								User user = ((UserController) controller)
										.getUser();
		...
		((ImageButton) findViewById(R.id.button_image_profile))
				.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
	...
	protected void onActivityResult(final int requestCode,
		...
		if (requestCode == INTENT_PICK_PICTURE && resultCode != RESULT_CANCELED
				...
				&& !data.getData().toString().trim().equals("")) {

			showDialog(DIALOG_PROGRESS);

			// assign the image
			try {
				Image image = new AndroidImage(data.getData(),
						getContentResolver());
	...
	protected Dialog onCreateDialog(final int id) {
		...
		case DIALOG_ERROR:
			return (new AlertDialog.Builder(this))
					.setPositiveButton(R.string.too_bad, null).setMessage("")
					.create();
		case DIALOG_SUCCESS:
			return (new AlertDialog.Builder(this))
					...
					.setPositiveButton(R.string.awesome, null).setMessage("")
					.create();
		}
		return null;
	}

	// handler to update the success and error dialog with the corresponding
	// message
	@Override
